 ...
 * @LastEditors  : 最后编写者
 * @LastEditTime : 2020-01-12 04:57:25
 * @Description: 文件描述
 */
module.exports = app => {
  ...
  router.get('/news/init', async (req, res) => {
    ...
    const newsTitle = [
      ...
      '新版本预告③｜模拟战新增英雄，小精灵还能打招呼~',
      '皮肤升级丨凤凰刺绣、云端御剑，征求你的意见',
      '新版本预告④｜√新英雄√新皮肤√新玩法，精彩全合集',
      '五岳皮肤研发全纪实丨鼠年限定首曝，还原五岳东方之美',
      '新版本预告②丨S18荣耀战令更新，昭君新衣抢先看！'
    ]
  ...
  router.get('/heroes/init', async (req, res) => {
    ...
    for (let cta of rawData) {
      if (cta.name === '热门') {
        continue
      }

      // 找到当前分类在数据库中对应的数据
      const category = await Category.findOne({ name: cta.name })

      cta.heroes = cta.heroes.map(hero => {
        hero.categories = [category]
        return hero
      })
      await Hero.insertMany(cta.heroes)
    }
  ...
  router.get('/news/list', async (req, res) => {
    ...
    cats.map(cta => {
      cta.newsList.map(news => {
        news.categoryName =
          cta.name === '热门' ? news.categories[0].name : cta.name
        return news
      })
      return cta
    })
